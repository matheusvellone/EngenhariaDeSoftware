...
class ToolbarComponent extends Component implements CakeEventListener {
    ...
    public function startup(Controller $controller) {
        ...
        DebugTimer::start(
                'controllerAction', __d('debug_kit', 'Controller action')
        );
        DebugMemory::record(
    ...
    public function beforeRedirect(Controller $controller, $url, $status = null, $exit = true) {
        ...
        DebugTimer::start(
                'processToolbar', __d('debug_kit', 'Processing toolbar state')
        );
    ...
    public function beforeRender(Controller $controller) {
        ...
        DebugTimer::start(
                'processToolbar', __d('debug_kit', 'Processing toolbar data')
        );
        $vars = $this->_gatherVars($controller);
        $this->_saveState($controller, $vars);
        ...
        if (isset($vars['variables']['content'])) {
            ...
            array_walk_recursive($vars['variables']['content'], function (&$item) {
                if (
                    ...
                    $item = sprintf(
                            'Unserializable object - %s. Error: %s in %s, line %s', get_class($item), $item, $item->getMessage(), $item->getFile(), $item->getLine()
                    );
                }
                return $item;
            });
