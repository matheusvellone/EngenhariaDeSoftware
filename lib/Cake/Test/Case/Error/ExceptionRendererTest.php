...
class ExceptionRendererTest extends CakeTestCase {
    ...
    public function testMissingPluginRenderSafe() {
        ...
        $response->expects($this->once())
                ...
                ->with($this->logicalAnd(
                                $this->logicalNot($this->stringContains('test plugin error500')), $this->stringContains('Not Found')
        ));
    ...
    public function testMissingPluginRenderSafeWithPlugin() {
        ...
        $response->expects($this->once())
                ->method('body')
                ->with($this->logicalAnd(
                                $this->stringContains('test plugin error500'), $this->stringContains('Not Found')
        ));
